...
declare -rx redis='/data1/redis/app/redis-5.0.10/src/redis-cli'
...
function getClusterNodes() {
  ...
  sleep 5
  declare -a cluster_nodes=$(${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER NODES | awk '{print $2"|"$1}');
  echo "Cluster Nodes:";
...
function clusterMeet() {
  ...
  echo "${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER MEET ${DEST_IP} ${DEST_PORT}"
   ${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER MEET ${DEST_IP} ${DEST_PORT}
}
...
function clusterSlotImport() {
  ...
  local hslot=${1}
  local src_id=${2}
  echo "${redis} -h ${DEST_IP} -p ${DEST_PORT} CLUSTER SETSLOT ${hslot} IMPORTING ${src_id}"
   ${redis} -h ${DEST_IP} -p ${DEST_PORT} CLUSTER SETSLOT ${hslot} IMPORTING ${src_id}
  return 0
}
...
function clusterSlotMigrate() {
  ...
  local hslot=${1}
  local dest_id=${2}
  echo "${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER SETSLOT ${hslot} MIGRATING ${dest_id}"
  ${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER SETSLOT ${hslot} MIGRATING ${dest_id}
  return 0
}
...
function clusterSetSlotNode() {
  ...
  local hslot=${1}
  local dest_id=${2}
  master_nodes=$(${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER NODES | grep master | awk '{print$2}')
  if [ -z "${master_nodes}" ]; then
    ...
    for master in ${master_nodes[@]}; do
      ...
      echo "${redis} -h ${ip} -p ${port} CLUSTER SETSLOT ${hslot} NODE ${dest_id}"
      ${redis} -h ${ip} -p ${port} CLUSTER SETSLOT ${hslot} NODE ${dest_id}
    done
  fi
  #echo "${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER SETSLOT ${hslot} NODE ${dest_id}"
  #echo "${redis} -h ${DEST_IP} -p ${DEST_PORT} CLUSTER SETSLOT ${hslot} NODE ${dest_id}"
  return 0
}
...
function migrateKey() {
  ...
  local hslot=${1}
  key_cnt=$(${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER COUNTKEYSINSLOT ${hslot} | awk '{print$1}')
  if [[ ${key_cnt} -ne 0 ]]; then
    keys=$(${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER GETKEYSINSLOT ${hslot} ${key_cnt} | awk '{print$1}')
    for key in ${keys[@]}; do
      echo "Migrating ${key} from ${hslot} to ${DEST_IP} ${DEST_PORT}"
      #key_slot=$(${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER keyslot ${key} | awk '{print$1}')
      #echo "redis-cli -h ${SRC_IP} -p ${SRC_PORT} MIGRATE ${DEST_IP} ${DEST_PORT} ${key} 0 2000"
      ${redis} -h ${SRC_IP} -p ${SRC_PORT} MIGRATE ${DEST_IP} ${DEST_PORT} ${key} 0 2000
    done
  else
...
function slotPusher() {
  ...
  local src_id=$(${redis} -h ${SRC_IP} -p ${SRC_PORT} CLUSTER NODES | grep "${SRC_IP}:${SRC_PORT}" | awk '{print$1}')
  local dest_id=$(${redis} -h ${DEST_IP} -p ${DEST_PORT} CLUSTER NODES | grep "${DEST_IP}:${DEST_PORT}" | awk '{print$1}')
...
function execute_main() {
  ...
  echo "startTime:" $startTime >>shard.log
  slotPusher
  endTime=`date +'%Y-%m-%d %H:%M:%S'`
  echo "endTime:" $endTime >>shard.log
}
